...
export default function Home() {
  ...
  // Fetch total customer count on mount
  useEffect(() => {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 pb-8">
      ...
      {/* ── Stats Row (2 cols mobile / 4 cols PC) ────────────────────────── */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-5">
        {[
          { label: "Clients", value: totalCustomers ?? "—", icon: Users, color: "text-blue-400", bg: "bg-blue-500/10" },
          { label: "Measurements", value: recentMeasurements.length, icon: Ruler, color: "text-emerald-400", bg: "bg-emerald-500/10" },
          { label: "Favorites", value: favorites.length, icon: Star, color: "text-amber-400", bg: "bg-amber-500/10" },
          { label: "Tools", value: ALL_TOOLS.length, icon: Grid3X3, color: "text-purple-400", bg: "bg-purple-500/10" },
        ].map(({ label, value, icon: Icon, color, bg }) => (
          <div key={label} className="flex items-center gap-2.5 px-3 py-2.5 rounded-2xl border border-border/50" style={{ background: "hsl(218,44%,11%)" }}>
            <div className={`w-8 h-8 rounded-lg ${bg} flex items-center justify-center shrink-0`}>
              <Icon size={15} className={color} />
            </div>
            <div className="min-w-0">
              <p className="text-sm font-black text-foreground leading-none">{value}</p>
              <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-wider mt-0.5 truncate">{label}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <section className="mb-8">
        ...
        {/* 3-column shortcut grid + bulk message */}
